...
mod tests {
    ...
    /// RFC #5890 patch ④ Gap A (endpoint-level failover): with
    /// `with_provider_models`, each provider in the chain is invoked with ITS
    /// OWN model string (resolved from the per-alias typed config), NOT the
    /// caller-supplied model. On the primary's failure the chain advances to
    /// the second provider, which is called with its own model. This is the
    /// behaviour the model-string `with_model_fallbacks` path could not
    /// provide — it shared the primary's construction (URI/key leaked).
    #[tokio::test]
    async fn provider_models_endpoint_failover_uses_own_model() {
        let primary = Arc::new(NativeModelAwareMock {
            calls: Arc::new(AtomicUsize::new(0)),
            models_seen: parking_lot::Mutex::new(Vec::new()),
            fail_models: vec!["primary-model"],
            response_text: "unused",
        });
        let fallback = Arc::new(NativeModelAwareMock {
            calls: Arc::new(AtomicUsize::new(0)),
            models_seen: parking_lot::Mutex::new(Vec::new()),
            fail_models: vec![],
            response_text: "ok from fallback",
        });
        ...
        let model_provider = ReliableModelProvider::new(
            ...
            vec![
                (
                    "primary".into(),
                    Box::new(primary.clone()) as Box<dyn ModelProvider>,
                ),
                (
                    "fallback".into(),
                    Box::new(fallback.clone()) as Box<dyn ModelProvider>,
                ),
            ],
            0, // no retries — force immediate provider failover
            1,
        )
        ...
        // The caller-supplied model must be ignored by BOTH providers in
        // favour of their own `provider_models` override.
        let result = model_provider
            .chat(request, "caller-supplied-ignored", Some(0.0))
            .await
            .unwrap();
        assert_eq!(result.text.as_deref(), Some("ok from fallback"));

        // Primary saw ONLY its own override model, never the caller's.
        let primary_seen = primary.models_seen.lock();
        assert_eq!(&*primary_seen, &["primary-model".to_string()]);
        // Fallback saw ONLY its own override model — not the caller's and not
        // the primary's (which is exactly what Gap A fixes).
        let fallback_seen = fallback.models_seen.lock();
        assert_eq!(&*fallback_seen, &["fallback-model".to_string()]);
    }

    /// RFC #5890 patch ④ Gap B (post-mortem attribution): on a FULL-chain
    /// bail (every provider fails), the post-mortem block records what was
    /// LAST attempted so the emission site's `take_last_provider_fallback()`
    /// can attribute the last-tried provider/model even on total failure.
    /// Without it the trace can't distinguish "primary failed, nothing
    /// recovered" from "primary failed, chain advanced to X, X also failed".
    #[tokio::test]
    async fn provider_models_full_chain_bail_records_last_attempt() {
        scope_provider_fallback(async {
            ...
            let fallback = Arc::new(NativeModelAwareMock {
                calls: Arc::new(AtomicUsize::new(0)),
                models_seen: parking_lot::Mutex::new(Vec::new()),
                fail_models: vec!["fallback-model"],
                response_text: "unused",
            });
            ...
            let model_provider = ReliableModelProvider::new(
                ...
                0, // no retries — exhaust each provider in one attempt
                1,
            )
            .with_provider_models(vec![
                Some("primary-model".to_string()),
            ...
            let err = model_provider
                .chat(request, "primary-model", Some(0.0))
                .await
                .expect_err("entire chain should fail");
            assert!(
                err.to_string().contains("All model_providers/models failed"),
                "unexpected error: {err}"
            );

            // Post-mortem: the LAST attempt (fallback provider + its own model)
            // is recorded even though everything failed.
            let fb = take_last_provider_fallback()
                .expect("post-mortem fallback should be recorded on full-chain bail");
            assert_eq!(fb.requested_provider, "primary");
            assert_eq!(fb.requested_model, "primary-model");
            assert_eq!(fb.actual_provider, "fallback");
            assert_eq!(fb.actual_model, "fallback-model");
        })
        .await;
    }
